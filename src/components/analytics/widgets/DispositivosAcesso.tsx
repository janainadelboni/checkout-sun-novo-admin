...
const dispositivos = [
  { key: '1', dispositivo: 'Desktop', color: '#2B4ACF', acessos: 5200, compras: 190, ticket: 223.68 },
  { key: '2', dispositivo: 'Celular', color: 'var(--ant-color-warning)', acessos: 4100, compras: 223, ticket: 180.50 },
  { key: '3', dispositivo: 'Tablet', color: '#CF2B9E', acessos: 800, compras: 19, ticket: 150.00 },
    ...
    },
    {
      ...
      render: (v: number) => (
        <span>
          ...
          <Typography.Text type="secondary" className="ml-1 !text-[14px]">({((v / totalAcessos) * 100).toFixed(0)}%)</Typography.Text>
        </span>
      ),
    },
    {
      ...
      render: (v: number) => (
        <span>
          {v.toLocaleString('pt-BR')}
          <Typography.Text type="secondary" className="ml-1 !text-[14px]">({((v / totalCompras) * 100).toFixed(0)}%)</Typography.Text>
        </span>
      ),
    },
    {
  ...
  return (
    <div className="border border-(--ant-color-split) rounded-lg p-6 h-full">
      ...
      <div className="flex justify-center mb-4">
        <PieChart segments={dispositivos.map((d) => {
          const pct = Math.round((d.acessos / totalAcessos) * 100)
          return {
            percent: pct,
            color: d.color,
            label: d.dispositivo,
            tooltip: `${d.dispositivo}: ${d.acessos.toLocaleString('pt-BR')} acessos (${pct}%)`,
          }
        })} />
      </div>
