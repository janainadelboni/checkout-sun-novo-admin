...
export function TaxaConversaoMetodo() {
  ...
  return (
    <div className="border border-(--ant-color-split) rounded-lg p-6 h-full">
      <Typography.Title level={5} className="mb-3">Transações</Typography.Title>
      <Segmented size="small" value={tab} onChange={(v) => setTab(v as string)} options={['Geral', 'Cartão', 'Boleto', 'Pix']} className="mb-4" />
      ...
      <div className="flex flex-col">
        {transacoes.map((t) => {
          ...
          return (
            <div key={t.label} className="flex items-center py-2 border-b border-(--ant-color-split) last:border-b-0">
              <div className="flex items-center gap-2 flex-1 min-w-0">
                <div className="w-3 h-3 rounded-sm shrink-0" style={{ backgroundColor: t.color }} />
                <Typography.Text className="truncate">{t.label}</Typography.Text>
              </div>
              <div className="w-[100px] text-right shrink-0 pl-3">
                <Typography.Text strong className="whitespace-nowrap">{t.valor}</Typography.Text>
              </div>
